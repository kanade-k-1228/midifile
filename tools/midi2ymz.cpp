...
void processMidiFile(MidiFile &midifile) {
  ...
  cout << "PROGMEM const uint16_t music[] = {" << endl;
  ...
  for (int i = 0; i < eventcount; i++) {
    ...
    if (0x80 <= command_nibble && command_nibble < 0xA0) {
      ...
      prev_time = abs_time;
      if (interval_time != 0) {
        cout << "0b0" << bitset<15>(interval_time) << "," << endl;
        count++;
      }
      int note_number = (int)(*ptr)[1];
      int velocity = (int)(*ptr)[2] >> 3; // 7 bit -> 4 bit
      cout << "0b1"
           << "00" << bitset<2>(track) << bitset<7>(note_number)
           << bitset<4>(velocity) << "," << endl;
      count++;
    }
  }
  cout << "};" << endl;
  cout << "const int len = " << count << ";" << endl;
}
